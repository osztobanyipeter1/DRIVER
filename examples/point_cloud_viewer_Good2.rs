...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    //let initial_orientation = UnitQuaternion::from_axis_angle(&Vector3::x_axis(), PI/2.0);
                ...
                match event {
                    GlassesEvent::AccGyro { accelerometer, gyroscope, timestamp: _ } => { //a gyorsulásmérő és a giroszkóp adatokat dolgozza fel
                        ...
                        if !calibrated {
                            ...
                            if calibration_samples.len() >= CALIBRATION_SAMPLES { //az első 1000 példát gyűjti a kalibrációhoz
                                ...
                                calibrated = true;
                                println!("Calibration complete. Biases - Accel: {:?}, Gyro: {:?}", accel_bias, gyro_bias); //kalibrációs eredményeket kiírja
                            }
                        ...
                        //println!("Gyro: {:?}, Accel: {:?}", corrected_gyro, corrected_acc);
                        ...
                        if corrected_acc.norm() > 9.5 && corrected_acc.norm() < 10.5 { //csak akkor fut le, ha a gyorsulásmérő értéke az adott számok között van
                            let normalized_acc = corrected_acc.normalize(); //normalizált gyorsulási vektor
                            let quat = *ori;
                            let current_up = quat * Vector3::y(); //jelenlegi fel irány a quaternionból
                        ...
                        let quat = *ori;
                        let (pitch, roll, yaw) = quat.euler_angles(); //euler szögek kinyerése a kvaternióból
                        println!("Orientation - Oldalra_Billentés (roll): {:.2}°, Fel_Le (pitch): {:.2}°, Balra_Jobbra (yaw): {:.2}°", roll.to_degrees(), pitch.to_degrees(), yaw.to_degrees());
                    }
                    _ => {}
                }
            }
...
fn handle_client(mut stream: TcpStream, orientation: Arc<Mutex<UnitQuaternion<f32>>>) { //orientation van megosztva
    ...
    let correction = UnitQuaternion::from_axis_angle(&Vector3::x_axis(), -PI/4.0);//ez hozzájárul, hogy kisebb legyen 90 fokkal az átküdött kód
